...
export default async function ModeEksplorasiPage({
  params,
}: {
  ...
  // If not a valid UUID, create a real sesi row for eksplorasi and redirect once
  if (!isValidUUID(rawId) && user) {
    ...
    if (newSesi?.id) {
      redirect(`/eksplorasi/${newSesi.id}`);
    }
  }
  ...
  const defaultSoalList = [
    {
      ...
      pertanyaan:
        "Sebuah pola susunan korek api membentuk barisan aritmetika: $3, 7, 11, 15, \\dots$. Tentukan jumlah korek api pada suku ke-10 ($U_{10}$)!",
    ...
    },
    {
      ...
      pertanyaan:
        "Eksplorasi Konsep: Jelaskan perbedaan mendasar antara **Barisan Aritmatika** dan **Barisan Geometri**, serta berikan masing-masing 1 contoh penerapan dalam kehidupan sehari-hari!",
  ...
  return (
    <SessionQuizClient
      sesiId={sesiId}
      jenisSesi="Eksplorasi (Kurasi AI)"
